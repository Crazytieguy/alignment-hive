...
**Keeps background-job markers** — When a reply ends with a `needs input:`, `result:` or `failed:` line (the markers Claude Code's job list reads), the TL;DR is asked to start with the same marker, so a background session keeps its needs-input or done status.
