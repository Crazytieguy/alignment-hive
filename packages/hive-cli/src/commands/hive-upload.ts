...
export async function hiveUpload(): Promise<number> {
  ...
  // Check consent windows to prevent uploading sessions from revocation gaps
  const consentHistory = await getConsentHistory(canonicalProject);
  const globalWindows = consentHistory
    ? computeConsentWindows(consentHistory.global)
    : [];
  const projectWindows = consentHistory
    ? computeConsentWindows(consentHistory.project)
    : [];

  const sessionsToUpload = allSessions.filter((session) => {
    if (!checkSessionEligibility(session, uploadedMap, excludedSet, consentMtime).eligible) {
      return false;
    }
    // Skip sessions whose mtime falls outside consent windows
    const mtime = session.mtime.getTime();
    if (!isInConsentWindow(mtime, globalWindows)) {
      if (process.env.DEBUG) {
        console.error(`[hive-upload] Skipping ${session.sessionId}: mtime outside global consent window`);
      }
      return false;
    }
    if (!isInConsentWindow(mtime, projectWindows)) {
      if (process.env.DEBUG) {
        console.error(`[hive-upload] Skipping ${session.sessionId}: mtime outside project consent window`);
      }
      return false;
    }
    return true;
